#!/usr/bin/env python3
"""
Render data/contributions.json as an animated, terminal-inspired
GitHub-style contribution heatmap SVG.
Outputs to assets/contrib-heatmap.svg.
"""
import datetime
import json
import os
...
# GitHub dark theme green gradient ramp
...
# Animation timing for diagonal cascade
COL_T = 0.016
ROW_T = 0.040
CELL_DUR = 0.40
...
""".strip()
    ...
    parts = [
        ...
        f'fill="none" stroke="{FRAME}" stroke-width="1" stroke-opacity="0.45"/>',
        f'<line x1="0" y1="{TITLEBAR_H}" x2="{canvas_w}" y2="{TITLEBAR_H}" stroke="{FRAME}" stroke-opacity="0.30"/>',
    ]
    ...
    parts.append(
        ...
        f'text-anchor="middle">cyril-p-jose@github: ~/contributions --graph</text>'
    )
    ...
    leg_x = canvas_w - PAD - (len(PALETTE) * CELL + 75)
    ...
    parts.append(
        ...
        f'<tspan fill="{MUTED}"> contributions in the past year</tspan></text>'
    )
    parts.append(
    ...
    parts.append(
        ...
        f'<tspan fill="{MUTED}">   &#183;   longest streak </tspan>'
        f'<tspan fill="{ACCENT}" font-weight="700">{ls} days</tspan></text>'
    )
    parts.append(
        f'<text x="{canvas_w - PAD}" y="{ly}" font-size="12" fill="{MUTED}" text-anchor="end">'
        f'best day <tspan fill="{GOLD}" font-weight="700">{best.get("count", 0)}</tspan> on {best.get("date", "-")}</text>'
    )
...
if __name__ == "__main__":
    ...
    with open(OUT_PATH, "w", encoding="utf-8") as f:
        f.write(svg)
    print(f"Rendered {OUT_PATH} ({len(svg)} bytes)")
